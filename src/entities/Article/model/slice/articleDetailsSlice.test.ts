...
describe('profileSlice.test.ts', () => {
    it('test set updateProfile', () => {
        const state: DeepPartial<ArticleDetailShema> = { data: article };
        expect(
            articleDetailsReducer(state as ArticleDetailShema, fetchArticleById.pending('', ''))
        ).toEqual({ data: article, error: undefined, isLoading: true });
    });
    it('test set update profile service fulfield', () => {
        const state: DeepPartial<ArticleDetailShema> = {
            isLoading: true,
            error: 'error',
        };
